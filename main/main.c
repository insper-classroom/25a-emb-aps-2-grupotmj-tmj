...
static QueueHandle_t xQueueEncoder;
...
void encoder_gpio_callback(uint gpio, uint32_t events)
{
    ...
    // Envia o valor do incremento para a fila ao invés de usar uma variável global
    BaseType_t xHigherPriorityTaskWoken = pdFALSE;
    xQueueSendFromISR(xQueueEncoder, &diff, &xHigherPriorityTaskWoken);
...
static void mpu_task(void *pvParameters)
{
    ...
    for (;;)
    {
        ...
        // Recebe os valores do encoder da fila
        while (xQueueReceive(xQueueEncoder, &encoder_diff, 0) == pdPASS)
        {
...
int main(void)
{
    ...
    xQueueEncoder = xQueueCreate(10, sizeof(int8_t));
